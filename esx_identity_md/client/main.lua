...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Vous devez créer votre premier personnage pour jouer' } })
	end
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	for theData, value in pairs(myIdentity) do
		if theData == "Prénom" or theData == "nom de famille" then
...
function verifyName(name)
	...
	if count ~= nameLength then
		return 'Votre nom de joueur contient des caractères spéciaux qui ne sont pas autorisés Sur la carte d\'identité.'
	end
